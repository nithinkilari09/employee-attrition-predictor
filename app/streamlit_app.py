...
if predict_btn:
    ...
    with col1:
        ...
        <div style='background-color:{risk_color};padding:20px;border-radius:10px;text-align:center;'>
            ...
            <p style='color:white;margin:0;font-size:12px;'>Decision threshold: {best_threshold:.2f}</p>
        </div>
    ...
    if job_level == 1:
        risk_factors.append("Entry level position - highest attrition group (27%)")
    if monthly_income < 3000:
        risk_factors.append("Low monthly income - below average for retained employees")
    if job_satisfaction <= 2:
        risk_factors.append("Low job satisfaction score")
    if years_at_company <= 2:
        risk_factors.append("Less than 2 years at company - critical retention window")
    if distance_from_home > 15:
        risk_factors.append("High distance from home - strong attrition correlator")
    if num_companies_worked > 4:
        risk_factors.append("Worked at many companies - history of job hopping")
    if marital_status == "Single":
        risk_factors.append("Single employees statistically leave more often")
    if work_life_balance <= 2:
        risk_factors.append("Poor work life balance reported")
    if stock_option_level == 0:
        risk_factors.append("No stock options - reduces financial retention incentive")
    ...
    with col1:
        ...
        st.error(f"Estimated replacement cost: **${replacement_cost:,}**")
